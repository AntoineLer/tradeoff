...
def linear_model(x_train, y_train, x_test, residual_error, number_irrelevant_variables):
    linear_estimators = [LinearRegression().fit(add_irrelevant_variables(x, number_irrelevant_variables), y) for x, y in zip(x_train, y_train)]
    linear_prediction = [estimator.predict(add_irrelevant_variables(x_test, number_irrelevant_variables)) for estimator in linear_estimators]
...
def non_linear_model(x_train, y_train, x_test, residual_error, number_irrelevant_variables):
    linear_estimators = [KNeighborsRegressor(n_neighbors=100).fit(add_irrelevant_variables(x, number_irrelevant_variables), y) for x, y in zip(x_train, y_train)]
    linear_prediction = [estimator.predict(add_irrelevant_variables(x_test, number_irrelevant_variables)) for estimator in linear_estimators]
...
def add_irrelevant_variables(X, number_irrelevant_variables, random_state=None):
    ...
    for i in range(X.shape[0]):
        variables[i] = np.concatenate(([X[i]], random.uniform(-10, 10, number_irrelevant_variables)))
...
if __name__ == "__main__":
    ...
    linear_model(x_train, y_train, x_test, residual_error, number_irrelevant_variables)
    non_linear_model(x_train, y_train, x_test, residual_error, number_irrelevant_variables)
